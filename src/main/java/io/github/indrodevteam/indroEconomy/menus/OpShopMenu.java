...
public class OpShopMenu extends Menu {
    ...
    public void handleMenu(InventoryClickEvent inventoryClickEvent) throws MenuManagerNotSetupException, MenuManagerException {
        ...
        if (!eco.hasAccount(player)) {
            player.sendMessage(LanguageLoader.TITLE.get() + LanguageLoader.ERROR_ACCOUNT_NOT_EXISTING.get());
        }
        ...
        switch (itemName) {
            case "Tier 1" -> {
                if (eco.hasAmount(player, 2500)) {
                    eco.changeBalance(player, -2500);
            ...
            case "Tier 2" -> {
                if (eco.hasAmount(player, 16500)) {
                    eco.changeBalance(player, -16500);
            ...
            case "Tier 3" -> {
                if (eco.hasAmount(player, 24550)) {
                    eco.changeBalance(player, -24500);
